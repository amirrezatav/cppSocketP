...
Queue* Queue::Upload(int socketid, string path, int QueueID , ChanheProgress chanheprogress)
{
	try
	{
		...
		if (!item->readfile.is_open())
			return nullptr;
		item->Lenght = item->readfile.seekg(0, std::ios::end).tellg();
...
void Queue::SetProgress(int read) 
{
	...
	this->Progress = (int)((this->Treansfered * 100) / this->Lenght);
...
{
	while (item->Running && item->Index < item->Lenght)
	{
		sendDoor.lock();

		long read = (item->Lenght - item->Index < SIZE_FileBuffer) ? item->Lenght - item->Index : SIZE_FileBuffer;
